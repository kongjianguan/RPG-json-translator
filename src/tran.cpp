...
void translatorThread(rapidjson::Value *rootp) {
    ...
    DoC("已完成" + to_string((int)fp) + "/" + to_string((int)col) + " | " +
        to_string(fp / col * 100) + "%");
}
void action(rapidjson::Value &root) {
    if (root.IsObject()) {
    ...
    if (inter == "logoff") {
        ...
        for (int i = 0; i != 65536; i += 2) {
            if (queue[i] == 1 or queue[i] == 2)
                cout << "queue[" << i << "]"
                     << (*reinterpret_cast<rapidjson::Value *>(queue[i + 1])).GetString() << endl;
        }
    } else if (inter == "laq") {
...
void userInteractThread(int argc, char *argv[]) {
    ...
    while (getline(cin, inter) && ifuirun)
        opt(inter);
}
